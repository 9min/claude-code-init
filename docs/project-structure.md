...
│   └── PULL_REQUEST_TEMPLATE.md # PR 템플릿 (TDD 체크리스트 포함)
├── .husky/                     # Git hooks (pre-commit, pre-push)
├── src/
│   ├── app/                    # 앱 진입점 (main.tsx, RouterProvider 설정)
│   ├── routes/                 # TanStack Router 파일 기반 라우트
│   │   ├── __root.tsx          # 루트 레이아웃 (공통 Provider, Layout)
│   │   ├── index.tsx           # / 경로
│   │   └── _authenticated/     # 인증 필요 라우트 그룹
│   ├── routeTree.gen.ts        # TanStack Router 자동 생성 (Git 추적)
│   ├── components/             # 컴포넌트
...
│   ├── contexts/               # React Context (AuthContext 등, 상세: state-management.md 참조)
│   ├── stores/                 # Zustand 스토어 (상세: state-management.md 참조)
...
├── docs/                       # 프로젝트 문서
├── public/                     # 정적 파일
├── .nvmrc                      # Node.js 버전 명시 (단일 진실 소스, 22 권장)
├── .env.example                # 환경변수 키 목록 (Git 추적)
├── .env.local                  # 로컬 환경변수 (Git 미추적)
├── index.html
├── biome.json                  # Biome 설정
├── .claudeignore                # AI 에이전트 탐색 제외 파일 목록
├── package.json
├── tailwind.config.js          # Tailwind CSS 설정 (색상, 폰트 커스텀)
├── tsconfig.json
├── vite.config.ts
...
```
src/routes/
├── __root.tsx              # 루트 레이아웃 (AuthProvider 등 공통 설정)
├── index.tsx               # / 경로
├── login.tsx               # /login 경로
└── _authenticated/         # 인증 필요 라우트 그룹 (언더스코어 = 레이아웃 그룹)
    ├── route.tsx           # 인증 체크 레이아웃 (ProtectedRoute)
    └── dashboard.tsx       # /dashboard 경로
```
...
// useAuth()를 RouterProvider 외부에서 호출해야 하므로 InnerApp으로 분리
function InnerApp() {
...
- `src/routeTree.gen.ts`는 `vite dev` 또는 `vite build` 실행 시 자동 생성된다. Git에 추적한다.
- 파일명 규칙: `_` 접두사는 레이아웃 그룹(URL에 포함 안 됨), `$`는 동적 파라미터(`$id.tsx` → `/items/123`).
...
> `import.meta.dirname`은 Node.js 22+(또는 21.2+)에서 사용 가능하다. `.nvmrc`에 `22`를 명시하면 된다. Node 20을 사용해야 한다면 `fileURLToPath(new URL("src", import.meta.url))`로 대체한다.
